...
class UserRoutes(object):
	...
	def on_post_register(self, req, res):
		...
		if 'name' not in body or 'username' not in body or 'password' not in body or 'question' not in body or 'answer' not in body:
			...
			cursor = db.cursor()
			sql = "INSERT INTO Users (Name, Email, Password, SecurityQuestion, SecurityAnswer) VALUES (%s, %s, %s, %s, %s, %s)"
			...
			cursor.execute(sql, (body['name'], body['email'], body['email'], hashedPassword, hashedQuestion, hashedAnswer))
			db.commit()
