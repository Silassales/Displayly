...
class UserRoutes(object):
	...
	def on_post_register(self, req, res):
		...
		if 'firstName' not in body or 'lastName' not in body or 'username' not in body or 'password' not in body or 'question' not in body or 'answer' not in body:
			...
			cursor = db.cursor()
			sql = "INSERT INTO Users (FirstName, LastName, Email, Password, SecurityQuestion, SecurityAnswer) VALUES (%s, %s, %s, %s, %s, %s)"
			...
			cursor.execute(sql, (body['firstName'], body['lastName'], body['email'], body['email'], hashedPassword, hashedQuestion, hashedAnswer))
			db.commit()
